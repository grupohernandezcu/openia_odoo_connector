...
class ProductProduct(models.Model):
    ...
    def create(self, vals):
        ...
        if not res.image_1920 and res.is_create_image:
            ...
            if image_base64:
                ...
                res.message_post(
                    body=(
                        f"Failed to generate image for product: '{res.name}'. "
                        f"Please make sure the AI configuration is properly set and the API key is valid. "
                    ),
    ...
    def write(self, vals):
        """Update image if the product name is changed and update image option is enabled."""
        if 'name' in vals and 'image_1920' not in vals:
            ...
            if is_update_image:
                ...
                if image_base64:
                    ...
                    self.message_post(
                        body=(
                            f"Failed to update image for product: '{vals['name']}'. "
                            f"Please make sure the AI configuration is properly set and the API key is valid. "
                        ),
                        message_type='notification'
                    )
